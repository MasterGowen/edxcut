...
class edXapi(object):
    ...
    def do_xblock_check_problem(self, url_name, responses=None, box_indexes=None):
        ...
        for inkey, response in list(
                responses.items()):  # if any of the responses is a list, that's for a multiple-choice problem allowing multiple reponses
            if isinstance(response, list):
                for ritem in response:
                    post_data.append(
                        ("%s[]" % inkey, ritem))  # duplicate keys, with [] to tell edx-platform server to recreate list
            else:
    ...
    def download_student_state_reports(self, module_ids=None, date_filter=None):
        ...
        for dlinfo in all_downloads:
            ...
            if '_student_state_from_' not in name:
                continue
            m = re.search(
                'student_state_from_(block-v1_.*\+block@.*)_([0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]-[0-9]+).csv',
                name)
            if not m:
    ...
    def get_course_advanced(self):
        '''
        Get course advanced settings
        '''
        self.ensure_studio_site()
        url = '%s/settings/advanced/%s' % (self.BASE, self.course_id)
        self.headers['Accept'] = "application/json"
        ret = self.ses.get(url, headers=self.headers)
        if not ret.status_code == 200:
            raise Exception("Failed to get course advanced settings, url=%s, err=%s" % (url, ret.status_code))
        return ret.json()
    ...
    def download_course_tarball(self):
        ...
        self.headers['Referer'] = url
        self.headers['Accept'] = 'application/json, text/javascript, */*; q=0.01'
        r3 = self.ses.post(url,
                           headers=self.headers)  # start the export process - tarball creation takes some time, poll until done
    ...
    def get_outline(self, usage_key=None):
        ...
        if not ret.status_code == 200:
            raise Exception(
                "Failed to get outline for %s via %s, ret(%s)=%s" % (usage_key, url, ret.status_code, ret.content))
        data = ret.json()
        if self.verbose > 1:
    ...
    def _get_block_by_name_from_outline(self, outline=None, block_name=None, block_category=None, path=None,
                                        nofail=False):
        '''
    ...
    def list_xblocks(self, outline=None, category=None, path=None):
        ...
        if not 'child_info' in outline:
            if outline['category'] == "vertical":
                ...
                raise Exception(
                    "[edXapi.list_xblocks] Missing child_info in outline %s" % json.dumps(outline, indent=4))
        for block in outline['child_info']['children']:
    ...
    def update_xblock(self, usage_key=None, data=None, path=None, create=False, category=None, extra_data=None,
        ...
        if not usage_key:
            if create:
                ...
                cnt = 0
                for name in path:
                    block_category = self.content_stages[
                        cnt] if cnt < 3 else category  # no default category after vertical
    ...
    def get_static_asset(self, fn, ofn=None, nofail=False):
        ...
        if not ret.status_code == 200:
            if nofail:
                ...
                raise Exception(
                    "[edXapi.get_static_asset] Failed to retrieve static asset %s, url=%s, ret status=%s" % (fn,
                                                                                                             static_asset_url,
                                                                                                             ret.status_code))
        if ofn:
    ...
    def upload_static_asset(self, fn):
        ...
        if not ret.status_code == 200:
            ...
            raise Exception(
                '[edXapi.upload_static_asset] Failed to upload %s, to url=%s, err=%s' % (fn, url, ret.status_code))
        if self.verbose:
    ...
    def delete_static_asset(self, asset_key=None, fn=None):
        ...
        if not ret.status_code in [200, 204]:
            raise Exception(
                '[edXapi.delete_static_asset] Failed to delete %s, using url=%s, err=%s' % (fn, url, ret.status_code))
        try:
            rj = ret.json()
        except Exception as err:
            if self.verbose:
    ...
    def get_video_transcript(self, url_name, videoid=None, lang="en", output_srt=False):
        ...
        if not ret.status_code == 200:
            raise Exception(
                '[edXapi.get_video_transcript] Failed to retrieve transcript for %s, via url=%s, err=%s' % (url_name,
                                                                                                            ret.request.url,
                                                                                                            ret.status_code))
        rdat = ret.json()
        if output_srt:
    ...
    def upload_video_transcript(self, tfn, url_name, videoid, tfp=None):
        ...
        if not ret.status_code == 200:
            ...
            raise Exception(
                '[edXapi.upload_transcript] Failed to upload %s, to url=%s, err=%s' % (tfn, url, ret.status_code))
        if self.verbose:
...
def test_get_xblock1(eapi_studio):
    ea = eapi_studio
    data = ea.get_xblock(
        path=["Example Week 2: Get Interactive", "Homework - Labs and Demos", "Code Grader", "Code Grader"])
