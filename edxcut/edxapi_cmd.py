...
"""
    ...
    parser.add_argument('-v', "--verbose", nargs=0,
                        ...
                        dest='verbose')
    parser.add_argument("-s", "--site-base-url", type=str, help="base url for course site, e.g. http://192.168.33.10",
                        default=None)
    ...
    parser.add_argument("-c", "--course_id", type=str, help="course_id, e.g. course-v1:edX+DemoX+Demo_Course",
                        default=None)
    parser.add_argument("--module-id-from-csv", type=str,
                        help="provide name of CSV file from which to get module_id values", default=None)
    parser.add_argument("-D", "--data-dir", type=str, help="directory where data is stored", default="DATA")
    parser.add_argument("-S", "--studio", help="specify that the edX site being accessed is a CMS studio sute",
                        action="store_true")
    parser.add_argument("-j", "--json-output", help="Dump result (eg from get_block) as JSON to stdout",
                        action="store_true")
    parser.add_argument("--json-output-html", help="Dump HTML portion of json result (eg from get_block) to stdout",
                        action="store_true")
    parser.add_argument("-t", "--type", type=str,
                        help="xblock content category type, used when creating new content xblock", default=None)
    parser.add_argument("--view", type=str, help="xblock view, used when getting xblock", default=None)
    parser.add_argument("-o", "--output-file-name", type=str, help="output file name to use, e.g. for get_asset",
                        default=None)
    parser.add_argument("-d", "--data", type=str, help="data to store (eg for xblock, when using create_block)",
                        default=None)
    parser.add_argument("--data-file", type=str,
                        help="filename with data to store (eg for xblock, when using create_block)", default=None)
    parser.add_argument("--extra-data", type=str, help="JSON string with extra data to store (for update_block)",
                        default=None)
    parser.add_argument("--videoid", type=str, help="videoid for get_video_transcript", default=None)
    parser.add_argument("--output-srt", help="have get_video_transcript output srt instead of srt.sjson",
                        action="store_true")
    ...
    parser.add_argument("--date", type=str,
                        help="date filter for selecting which files to download, in YYYY-MM-DD format", default=None)
    ...
    if args.cmd == "list_reports":
        ...
        if args.extra_data:
            try:
                ...
                sys.exit(-1)
        ret = ea.update_xblock(path=args.ifn, category=args.type, data=args.data, create=args.create,
